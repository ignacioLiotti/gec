...
function TableRowInner<Row extends FormTableRow>({
	...
	// Track dirty cells locally - will be updated by individual cells
	const [dirtyCellIds, setDirtyCellIds] = useState<Set<string>>(() => new Set());

	const handleCellDirtyChange = useCallback((columnId: string, isDirty: boolean) => {
		setDirtyCellIds(prev => {
	...
	const isRowDirty = dirtyCellIds.size > 0;
					...
					return (
						<td key={cell.id} {...getStickyProps(columnId, baseClassName)}>
							<MemoizedTableCell
								...
								onDirtyChange={handleCellDirtyChange}
