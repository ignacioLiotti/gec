      ...
      if (!folder.isOcr || mode === "files") {
        ...
        const targetFolderPath =
          normalizedFolderPath.length === 0
            ? obraId
            : normalizedFolderPath === obraId || normalizedFolderPath.startsWith(`${obraId}/`)
              ? normalizedFolderPath
              : `${obraId}/${normalizedFolderPath}`;

        const uploadForm = new FormData();
        uploadForm.append("file", file);
        uploadForm.append("folderPath", targetFolderPath);
    ...
    <div className="space-y-1">
      ...
      {localActions.length === 0 && (
        <p className="rounded-lg border border-dashed px-3 py-4 text-xs text-muted-foreground">
          No hay acciones rapidas todavia. Crea una para esta obra desde el boton Crear.
        </p>
      )}
    </div>
  );
