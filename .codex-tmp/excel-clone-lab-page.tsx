...
import { Badge, Button, Input, cn } from "@acme/ui"
  ...
  return (
    ...
    >
      ...
      <defs>
        ...
        >
          <stop stopColor={firstStopColor ?? "#79716b"} />
          <stop offset="1" stopColor={secondStopColor ?? "#57534d"} />
        </linearGradient>
      </defs>
    </svg>
  );
      ...
      <div className="group flex h-[105px] shrink-0 flex-col items-center justify-end gap-2">
        <button
          ...
          className={cn(
            ...
            isOcrEnabled ? "bg-gradient-to-b from-amber-500 to-amber-700" : "bg-gradient-to-b from-stone-500 to-stone-700"
          )}
          title={item[0]}
        >
          <div className="flex h-full w-full flex-col items-center justify-end">
            {hasContent ? (
              <span className="absolute -top-2 left-1/2 h-[80px] w-[100px] -translate-x-1/2 border bg-gradient-to-b from-stone-100 to-stone-200 transition-all duration-200 ease-in-out group-hover:-top-4" />
            ) : null}
            <FolderFront
