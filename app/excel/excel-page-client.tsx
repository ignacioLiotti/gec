...
type ExcelPageClientProps = {
	initialMainTableColumnsConfig: MainTableColumnConfig[] | null;
	initialObras: Parameters<typeof mapObraToDetailRow>[0][];
};

function mapDetailRowToMobileObra(row: ObrasDetalleRow): ObraListItem {
	return {
		id: row.id,
		n: typeof row.n === "number" ? row.n : row.n ? Number(row.n) : null,
		designacionYUbicacion: row.designacionYUbicacion ?? null,
		entidadContratante: row.entidadContratante ?? null,
		porcentaje:
...
export default function ExcelPageClient({
	...
	const [mobileObras, setMobileObras] = useState<ObraListItem[]>(() =>
		initialObras.map(mapObraToDetailRow).map(mapDetailRowToMobileObra)
	);
				...
				(async () => {
					try {
						...
						try {
							...
							console.error("No se pudieron cargar las obras existentes para calcular el N°", existingError);
						}
						...
						const updates = validRows.map((row) => {
							...
							toast.message(`Se asignaron Nro consecutivos desde ${currentMaxN + 1} para agregar las obras al final`);
	...
	useEffect(() => {
		...
		if (hydratedRows != null) {
			setMobileObras(hydratedRows.map(mapDetailRowToMobileObra));
		...
		const loadObras = async () => {
			try {
				...
				const obras = Array.isArray(payload.detalleObras)
					? (payload.detalleObras as Parameters<typeof mapObraToDetailRow>[0][])
					: [];
				if (!cancelled) {
					setMobileObras(obras.map(mapObraToDetailRow).map(mapDetailRowToMobileObra));
				}
			} catch (error) {
				console.error(error);
				if (!cancelled) {
