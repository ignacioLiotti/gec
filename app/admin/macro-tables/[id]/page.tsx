...
export default function EditMacroTablePage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      await fetchMainTableColumns();
      await fetchSidebarConfig();
      setIsLoading(false);
    };
    void load();
  }, [fetchMacroTable, fetchMainTableColumns, fetchObrasWithTablas, fetchTemplates, fetchSidebarConfig]);
  ...
  useEffect(() => {
    if (obras.length === 0 || selectedSources.length === 0) return;

    const tablaById = new Map<string, { obra: Obra; tabla: ObraTabla }>();
    for (const obra of obras) {
      for (const tabla of obra.tablas) {
        tablaById.set(tabla.id, { obra, tabla });
      }
    }
    ...
    setSelectedSources((prev) => {
      ...
      const next = prev.map((source) => {
        const resolved = tablaById.get(source.tablaId);
        if (!resolved) return source;
        const { obra, tabla } = resolved;
        const sameColumns =
          source.columns.length === tabla.columns.length &&
          source.columns.every((column, index) => column.fieldKey === tabla.columns[index]?.fieldKey);
        const sameMetadata =
          source.obraId === obra.id &&
          source.obraName === obra.designacionYUbicacion &&
          source.tablaName === tabla.name;
        if (sameColumns && sameMetadata) return source;
        changed = true;
        return {
          ...source,
          obraId: obra.id,
  ...
  useEffect(() => {
    if (
      ...
      selectedSources.some((source) => source.columns.length === 0)
    ) {
      return;
    }
    ...
    setColumns((prev) => {
      if (selectedSources.length === 0) return prev;
  ...
  }, [availableSourceColumns, selectedSources, selectedTableSourceColumns]);
  ...
  const updateColumnConfig = (colId: string, updates: Record<string, unknown>) => {
    setColumns((prev) =>
      prev.map((column) =>
        column.id === colId
