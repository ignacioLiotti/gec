...
export default async function TenantSetupPage() {
	...
	const highestObraNumber = Number(obrasResult.data?.[0]?.n ?? 0);
	const nextObraNumber = Number.isFinite(highestObraNumber)
		? highestObraNumber + 1
		: 1;
	...
	const steps: SetupStep[] = [
		...
		},
		{
			id: "first-obra",
			title: "Primera obra",
			description: "Cargá la obra con la que vas a empezar a trabajar.",
			ready: setupStatus.firstObraReady,
			href: "/excel?setup=first-obra",
			action: obraCount > 0 ? "Ver mis obras" : "Crear mi primera obra",
			icon: HardHat,
		},
		{
			id: "team",
					...
					return (
						...
						>
							...
							{step.id === "first-obra" && !step.ready ? (
								<FirstObraDialog nextObraNumber={nextObraNumber} />
							) : (
