		...
		<svg
			...
			<defs>
				...
				<radialGradient
					...
					<stop
						offset='0%'
						stopColor={variant === "financiero" ? "#ff8a3d" : "#ff5800"}
						stopOpacity='0.32'
					/>
					<stop
						offset='55%'
		...
		<section className={`hero hero-${variant || "default"}`}>
			...
			<div className={isOperativo ? "hero-copy-wrap" : "wrap-narrow"}>
				...
				<div className='hero-cta-row'>
					<button
						className={
							variant === "financiero" ? "btn btn-orange" : "btn btn-primary"
						}>
...
function StatsStrip({ items }) {
	return (
		<section className='stats w-full! bg-[#f7f5f1] flex justify-center'>
			<div className='stats-inner max-w-[73vw]'>
				{items.map((it, i) => (
					<div
						className='stat-cell'
						key={i}>
						<div className='stat-eyebrow'>{it.eyebrow}</div>
						<div className={"stat-value" + (it.orange ? " orange" : "")}>
							{it.value}
						</div>
...
function Footer({ variant }) {
	const links = [
		{
			h: "Producto",
			items: ["Obras", "Documentos", "Macro Tablas", "Reportes", "Permisos"],
		},
		{
			h: "Enfoques",
			items: ["Operativo", "Financiero", "Dirección", "Obra pública"],
		},
		{
			h: "Recursos",
			items: ["Documentación", "Changelog", "Estado", "Contacto"],
		},
	];
	return (
		<footer
			...
			<div
				...
				<div
					...
					<div>
						...
						<p
							...
							Sistema operativo para constructoras. Centraliza obras,
							documentos, certificados, tablas, permisos y reportes en una
							plataforma configurable por organización.
						</p>
					</div>
					{links.map((col) => (
						<div key={col.h}>
				...
				<div
					...
					<div className='footer-meta'>
						Sintesis Cloud Solutions &middot; Control de Ingeniería de Próxima
						Generación
					</div>
					<div className='footer-meta'>
						Buenos Aires &middot; Corrientes &middot; Córdoba
					</div>
				</div>
			</div>
		</footer>
